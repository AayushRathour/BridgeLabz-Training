...
public class NoteController {
    ...
    public NoteResponse createNote(
        ...
        return noteMapper.toResponse(noteService.createNote(request, email));
    }
    ...
    public List<NoteResponse> getAllNotes(Authentication authentication) {
        ...
        return noteMapper.toResponseList(noteService.getUserNotes(email));
    }
    ...
    public NoteResponse getNote(
        ...
        return noteMapper.toResponse(noteService.getNote(id, email));
    }
    ...
    public NoteResponse updateNote(
        ...
        return noteMapper.toResponse(noteService.updateNote(id, request, email));
    }
    ...
    public NoteResponse togglePin(
    	...
    	return noteMapper.toResponse(noteService.togglePin(id, email));
    }
    
    // Archives or unarchives the logged-in user's note
    @PutMapping("/{id}/archive")
    public NoteResponse toggleArchive( @PathVariable Long id, Authentication authentication) {
    	String email =authentication.getName();
    	return noteMapper.toResponse(noteService.toggleArchive(id, email));
    }
    
    // Moves a note to trash or restores it
    @PutMapping("/{id}/trash")
    public NoteResponse toggelTrash(@PathVariable Long id, Authentication authentication) {
    	String email = authentication.getName();
    	
    	return noteMapper.toResponse(noteService.toggelTrash(id, email));
    }
    ...
    public NoteResponse setReminder(
    		@PathVariable Long id,
    	...
    	return noteMapper.toResponse(noteService.setReminder(id, request.getReminderAt(), email));
    }
    
    // Added Search endpoint, Searches the logged-in user's notes by title or conten
    @GetMapping("/search")
    public List<NoteResponse> search(@RequestParam String keyword, Authentication authentication){
    	
    	String email = authentication.getName();
    	return noteMapper.toResponseList(noteService.searchNote(keyword, email));
    }
    ...
    public List<NoteResponse> filter(
    	...
    	return noteMapper.toResponseList(noteService.filterNotes(archived, trashed, email));
    }
}
